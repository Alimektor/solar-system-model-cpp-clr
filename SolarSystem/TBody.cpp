...
TBody::TBody(TBody ^center, int radius, float rotation_speed, int size, Color color)
{
	...
	m_color = color;
	m_angle = 0;
	int x = cos(m_angle) * m_radius;
	int y = sin(m_angle) * m_radius;
	m_current_coordinate = gcnew Coordinate(
...
{
	int size = m_size * m_size_ñoefficient;
...
void TBody::rotate()
{
	...
	int x = cos(m_angle) * m_radius * m_radius_ñoefficient;
	int y = sin(m_angle) * m_radius * m_radius_ñoefficient;
	m_current_coordinate = gcnew Coordinate(
		m_center->get_current_coordinate()->get_x() + x,
		m_center->get_current_coordinate()->get_y() + y
	);
...
void TBody::change_size(int ñoefficient)
{
	m_size_ñoefficient = ñoefficient;

	for each (TBody^ body in m_satellite_list)
	{
		body->change_size(ñoefficient);
	}
}

void TBody::change_radius(int ñoefficient)
{
	m_radius_ñoefficient = ñoefficient;

	for each (TBody^ body in m_satellite_list)
	{
		body->change_radius(ñoefficient);
	}
}
